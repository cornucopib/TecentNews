<template>
  <div
    class="gt-swiper"
    :class="{'gt-swiper-vertical': isVertical, 'gt-swiper-fade': !isSlide, 'disabled': !isInitial}"
  ...
  >
    <div class="gt-swiper-box">
      <div class="gt-swiper-container">
        <slot></slot>
      </div>
    </div>
    <div
      class="gt-swiper-indicators"
      :class="{'disabled': !hasDots}"
      v-if="oItemCount > 1 && hasDots"
    >
      <template v-for="index in oItemCount">
        <div
          class="gt-swiper-indicator"
          :key="index"
          :class="{ 'gt-swiper-indicator-active': index - 1 === realIndex }"
        ></div>
      </template>
    </div>
  </div>
